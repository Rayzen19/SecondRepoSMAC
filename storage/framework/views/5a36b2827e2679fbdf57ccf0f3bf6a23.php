        ...
        <div class="card">
            ...
            <div class="card-body p-0">
                <ul id="conversation-list" class="list-group list-group-flush">
                    ...
                    <li class="list-group-item conversation-item <?php echo e(isset($unreadCounts[$p->id]) && $unreadCounts[$p->id] > 0 ? 'has-unread' : ''); ?>" 
                        data-user-id="<?php echo e($p->id); ?>" 
                        data-user-name="<?php echo e($p->name); ?>" 
                        data-user-email="<?php echo e($p->email); ?>"
                        data-unread-count="<?php echo e($unreadCounts[$p->id] ?? 0); ?>">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <strong class="conversation-name"><?php echo e($p->name); ?></strong>
                                <div class="small text-muted"><?php echo e($p->email); ?></div>
                            </div>
                            <div class="text-end">
                                <?php if(isset($unreadCounts[$p->id]) && $unreadCounts[$p->id] > 0): ?>
                                    <span class="unread-badge" id="unread-<?php echo e($p->id); ?>"><?php echo e($unreadCounts[$p->id]); ?></span>
                                <?php else: ?>
                                    <span class="unread-badge d-none" id="unread-<?php echo e($p->id); ?>">0</span>
                                <?php endif; ?>
                            </div>
                        </div>
                    </li>
...
<script>
    (function(){
        ...
        function loadConversation(userId, scrollToBottom = true) {
            fetch("<?php echo e(url('/teacher/messenger/conversation/')); ?>/" + userId)
                .then(r => r.json())
                .then(data => {
        ...
        sendForm.addEventListener('submit', function(e){
            ...
            fetch("<?php echo e(route('teacher.messages.sendConversation')); ?>", {
                method: 'POST',
                headers: {
        ...
        function loadAllUsers() {
            fetch("<?php echo e(url('/teacher/api/all-users')); ?>")
                .then(r => r.json())
                .then(users => {
                    allUsers = users;
        ...
        function updateUnreadCounts() {
            fetch("<?php echo e(route('teacher.api.unread-counts-by-partner')); ?>")
                .then(r => r.json())
                .then(data => {
...
<?php echo $__env->make('teacher.components.template', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\NEWSMAC\resources\views/teacher/messages/messenger.blade.php ENDPATH**/ ?>