...
(function () {
	...
	function init() {
		...
		// If a submenu link is active, open its parent only if not already marked as open (subdrop)
		$('.sidebar-menu ul li.submenu a.active')
			...
			.each(function() {
				...
				if (!$header.hasClass('subdrop')) {
					$header.trigger('click');
				} else {
					// Ensure already-open server-side menus stay visible without toggling closed
					$header.next('ul').show();
				}
			});
	}
