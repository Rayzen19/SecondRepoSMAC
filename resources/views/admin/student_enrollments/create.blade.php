...
<div class="content">
    <div class="card">
        <div class="card-body p-5">
            <form method="POST" action="{{ route('admin.student-enrollments.store') }}">
                @csrf
                <div class="row g-3">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="academic_year_id" class="form-select @error('academic_year_id') is-invalid @enderror">
                            ...
                            @foreach($academicYears->where('is_active', true) as $ay)
                            <option value="{{ $ay->id }}" {{ (old('academic_year_id', request('academic_year_id')) == $ay->id) ? 'selected' : '' }}>{{ $ay->display_name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="strand_id" class="form-select @error('strand_id') is-invalid @enderror">
                            ...
                            <option value="{{ $st->id }}" {{ old('strand_id') == $st->id ? 'selected' : '' }}>{{ $st->code }} — {{ $st->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="academic_year_strand_section_id" class="form-select @error('academic_year_strand_section_id') is-invalid @enderror">
                            ...
                            <option value="{{ $assn->id }}" {{ old('academic_year_strand_section_id') == $assn->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" class="form-select @error('status') is-invalid @enderror">
                            ...
                            <option value="{{ $st }}" {{ old('status','enrolled') == $st ? 'selected' : '' }}>{{ ucwords($st) }}</option>
                            @endforeach
                        </select>
