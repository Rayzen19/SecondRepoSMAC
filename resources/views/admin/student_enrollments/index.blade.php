@extends('admin.components.template')

@section('breadcrumb')
<div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
    <div class="my-auto mb-2">
        <div class="d-flex align-items-start">
            <div class="me-3">
                <h2 class="mb-1 h4">
                    <i class="ti ti-users-group me-2"></i>
                    Enrollments
                </h2>
            </div>
        </div>
        <nav>
            <ol class="breadcrumb mb-0">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><i class="ti ti-smart-home"></i></a></li>
                <li class="breadcrumb-item active" aria-current="page">Enrollments</li>
            </ol>
        </nav>
    </div>
    <div class="d-flex my-xl-auto right-content align-items-center flex-wrap ">
        <div class="mb-2">
            <a href="{{ route('admin.student-enrollments.create') }}" class="btn btn-primary d-flex align-items-center"><i class="ti ti-plus me-2"></i>New Enrollment</a>
        </div>
    </div>
</div>
@endsection

@section('content')
<div class="content">
    @if(session('success'))
        <div class="alert alert-success alert-dismissible fade show" role="alert">
            {{ session('success') }}
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"><i class="ti ti-x"></i></button>
        </div>
    @endif

    <div class="card">
        <div class="card-body p-0">
            <div class="p-3 border-bottom bg-light">
                <form method="GET" action="{{ route('admin.student-enrollments.index') }}" class="row g-2">
                    <div class="col-md-3">
                        <select name="academic_year_id" class="form-select">
                            <option value="">All Academic Years</option>
                            @isset($academicYears)
                                @foreach($academicYears as $ay)
                                    <option value="{{ $ay->id }}" {{ request('academic_year_id') == $ay->id ? 'selected' : '' }}>{{ $ay->display_name }}</option>
                                @endforeach
                            @endisset
                        </select>
                    </div>
                    <div class="col-md-4">
                        <select name="strand_id" class="form-select">
                            <option value="">All Strands</option>
                            @isset($strands)
                                @foreach($strands as $st)
                                    <option value="{{ $st->id }}" {{ request('strand_id') == $st->id ? 'selected' : '' }}>{{ $st->code }} — {{ $st->name }}</option>
                                @endforeach
                            @endisset
                        </select>
                    </div>
                    <div class="col-md-3">
                        <select name="status" class="form-select">
                            <option value="">All Statuses</option>
                            @foreach(['enrolled','dropped','completed'] as $st)
                                <option value="{{ $st }}" {{ request('status') === $st ? 'selected' : '' }}>{{ ucwords($st) }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-1 d-grid">
                        <button class="btn btn-outline-primary" type="submit"><i class="ti ti-filter me-1"></i>Filter</button>
                    </div>
                    <div class="col-md-1 d-grid">
                        <a href="{{ route('admin.student-enrollments.index') }}" class="btn btn-outline-secondary"><i class="ti ti-refresh me-1"></i>Reset</a>
                    </div>
                </form>
            </div>
            <div class="table-responsive">
                <table class="table table-striped table-bordered mb-0">
                    <thead>
                        <tr>
                            <th style="width: 180px;">Reg. #</th>
                            <th style="width: 200px;">Student #</th>
                            <th>Name</th>
                            <th>AY</th>
                            <th>Strand</th>
                            <th>Section</th>
                            <th>Status</th>
                            <th style="width: 140px;">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($enrollments as $row)
                            <tr>
                                <td>{{ $row->registration_number }}</td>
                                <td>{{ $row->student?->student_number }}</td>
                                <td>{{ $row->student?->name }}</td>
                                <td>{{ $row->academicYear?->display_name }}</td>
                                <td>{{ $row->strand?->code }}</td>
                                <td>{{ $row->academicYearStrandSection?->section?->grade }} {{ $row->academicYearStrandSection?->section?->name }}</td>
                                <td>
                                    @php($badge = $row->status === 'enrolled' ? 'success' : ($row->status === 'completed' ? 'primary' : 'secondary'))
                                    <span class="badge bg-{{ $badge }}">{{ ucfirst($row->status) }}</span>
                                </td>
                                <td>
                                    <a href="{{ route('admin.student-enrollments.show', $row) }}" class="btn btn-sm btn-outline-primary">View</a>
                                    <a href="{{ route('admin.student-enrollments.edit', $row) }}" class="btn btn-sm btn-outline-secondary">Edit</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="7" class="text-center">No enrollments found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
            <div class="p-3">
                {{ $enrollments->links() }}
            </div>
        </div>
    </div>
</div>
@endsection
