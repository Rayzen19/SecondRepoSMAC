...
<div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
    <div class="my-auto mb-2">
        <h2 class="mb-1">Edit Academic Year</h2>
        <nav>
...
<div class="content">
    <div class="card">
        <div class="card-body p-5">
            <form action="{{ route('admin.academic-years.update', $year) }}" method="POST" class="max-w-2xl mx-auto bg-white p-6">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <select name="semester" class="form-select">
                        ...
                        @php $sem = old('semester', $year->semester); @endphp
                        <option value="1st" @selected($sem==='1st')>1st</option>
                        <option value="2nd" @selected($sem==='2nd')>2nd</option>
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <select name="academic_status" class="form-select" required>
                        @php $status = old('academic_status', $year->academic_status); @endphp
                        <option value="pending" @selected($status==='pending')>Pending</option>
                        <option value="completed" @selected($status==='completed')>Completed</option>
                        <option value="ongoing enrollment" @selected($status==='ongoing enrollment')>Ongoing Enrollment</option>
                        <option value="ongoing school year" @selected($status==='ongoing school year')>Ongoing School Year</option>
                    </select>
                    @error('academic_status')<div class="text-danger small">{{ $message }}</div>@enderror
                </div>

                <div class="form-check form-switch mb-4">
                    <input class="form-check-input" type="checkbox" role="switch" name="is_active" id="is_active" value="1" {{ old('is_active', $year->is_active) ? 'checked' : '' }}>
